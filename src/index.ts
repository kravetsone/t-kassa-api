...
> {
	...
	on<const Filter extends UpdateFilter<any>>(
		...
		if (!handler && typeof filtersOrHandler === "function")
			// @ts-expect-error
			this.listeners.push(filtersOrHandler);
	 ...
	 *
	 *
	 * @tags Стандартный платеж, Методы, которые используются в WebView
	 ...
	 *
	 * > По операциям **SberPay** рекуррентные списания недоступны.
	 * Для платежей, проведённых этим способом, параметр `RebillId` не формируется и не возвращается.
	 *
    ...
    */ paymentId: paths["/v2/SberPay/transactions/{paymentId}/link"]["get"]["parameters"]["path"]["paymentId"],
	...
	/**
	 * Справку по конкретной операции можно получить на: <br><ul><li> URL-сервиса, который развернут на вашей стороне. Файл со справкой придет в формате `base64`;<\/li><li> электронную почту.<\/li><\/ul> Чтобы сформировать токен, нужно использовать только <code>PASSWORD<\/code> и <code>TERMINAL_KEY<\/code>.
	 *
	 * @tags Стандартный платеж

...
export interface paths {
	...
	"/v2/getConfirmOperation": {
		 ...
		 * @description Справку по конкретной операции можно получить через 24 часа на: <br> 1. URL-сервиса, развернутого на своей стороне. <br> 2. Электронную почту. <br> Для формирования токена необходимо использовать только <b>PASSWORD</b> и <b>TERMINAL_KEY</b>
		 */
...
export interface components {
	schemas: {
		...
		Init_FULL: {
			...
			Amount: number;
			/**
			 * @description Идентификатор заказа в системе Мерчанта <br> Идентификатор должен быть уникальным для каждой операции
			 * @example 21050
			 */
			OrderId: string;
			/**
